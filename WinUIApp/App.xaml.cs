	...
	public sealed partial class App : Application
	{
		...
		public App()
		{
			...
			_host = Host.CreateDefaultBuilder()
				.ConfigureAppConfiguration(builder => builder.AddJsonFileWithValidation(configFileModel, true, reloadOnChange: true).AddJson(lastUpdateFile, true)
#if DEBUG
				.AddJsonFile($"appsettings.{Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") ?? string.Empty}.json", true, true)
#endif
				)
				.AddFileModel(configFileModel, static (services, section) => services.Configure<ConfigModel>(section))
				.AddFileModel(lastUpdateFileModel, static (services, section) => services.Configure<LastUpdateModel>(section))
				.ConfigureServices(static services => services.AddSingleton(AppNotificationManager.Default).AddSingleton<NotificationHelper>().AddHostedService<MainService>().AddHostedService<CrunchyrollService>().AddScoped<Settings>().AddScoped(static _ => new HttpClientHandler()).ConfigureHttpClientDefaults(static builder => builder.ConfigureHttpClient(static client => client.DefaultRequestHeaders.UserAgent.ParseAdd("chrome")).ConfigurePrimaryHttpMessageHandler<HttpClientHandler>()))
				.ConfigureSavableJson<IConfigModel>()
				.ConfigureSavableJson<ILastUpdateModel>()
